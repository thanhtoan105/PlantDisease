...
const CitySearchModal = ({ 
  ...
  const { currentWeather, getWeatherForCity, searchCities: searchCitiesFromContext } = useWeather();
  ...
  const handleCityPress = async (city) => {
    ...
    try {
      const weatherData = await getWeatherForCity(city.latitude, city.longitude);
  ...
  const handleAddCity = () => {
    if (selectedCity) {
      setAddedCities([...addedCities, selectedCity]);
  ...
  const getConditionDescription = (weatherData) => {
    ...
    return weatherData.condition.description.charAt(0).toUpperCase() + 
        ...
        <View style={styles.currentLocationCard}>
          <View style={styles.currentLocationInfo}>
            <View style={styles.locationHeader}>
              <Icon name="my-location" size={16} color={AppColors.primaryGreen} />
            ...
            <View style={styles.weatherInfo}>
              <SvgXml xml={getWeatherIcon(currentWeather.condition)} width={24} height={24} />
              <Text style={styles.temperature}>{getTemperature(currentWeather)}°</Text>
        ...
        {addedCities.map((city, index) => (
          <View key={index} style={styles.addedCityCard}>
            ...
            <TouchableOpacity 
              ...
              onPress={() => {
                setAddedCities(addedCities.filter((_, i) => i !== index));
              }}
            >
  ...
  const renderSearchResults = () => {
    ...
    return (
      <FlatList
        data={searchResults}
        keyExtractor={(item) => `${item.name}-${item.country}-${item.latitude}-${item.longitude}`}
        renderItem={({ item }) => (
          <TouchableOpacity
            ...
            <View style={styles.weatherPreviewContent}>
              ...
              <View style={styles.weatherStats}>
                <View style={styles.statItem}>
                  <Icon name="water-drop" size={16} color={AppColors.secondaryGreen} />
                  <Text style={styles.statText}>{getHumidity(cityWeatherPreview)}%</Text>
                </View>
                <View style={styles.statItem}>
                  <Icon name="air" size={16} color={AppColors.accentOrange} />
                  <Text style={styles.statText}>{getWindSpeed(cityWeatherPreview)} km/h</Text>
                </View>
              </View>
            </View>
          ) : (
          ...
          <View style={styles.content}>
            ...
            <View style={styles.searchSection}>
              ...
              <View style={styles.searchContainer}>
                <Icon name="search" size={20} color={AppColors.mediumGray} />
                <TextInput
                  ref={searchInputRef}
