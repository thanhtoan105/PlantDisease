...
const ResultCard = ({
	...
	return (
		<CustomCard style={styles.resultCard}>
			...
			{databaseInfo && onViewDetails && (
				<View style={styles.detailsSection}>
					<CustomButton
						title='View Detailed Information'
						type={ButtonType.SECONDARY}
						onPress={() => onViewDetails(databaseInfo)}
...
const ResultsScreen = () => {
	...
	useEffect(() => {
		const analyzeImage = async () => {
			try {
				...
				const result = await PlantDiseaseService.analyzeImage(
					imageUri,
					plantType,
				);
				...
				if (result.data.analysisMethod === 'tensorflow_lite') {
						console.log('Analysis completed using TensorFlow Lite model');
					if (result.data.enhancedWithDatabase) {
						console.log('Disease information enhanced with database data');
					}
				} else {
					console.log('Analysis completed using fallback method');
					}
				} else {
	...
	const handleViewDiseaseDetails = (diseaseInfo) => {
		// Navigate to disease detail screen with database information
		navigation.navigate('DiseaseDetail', { disease: diseaseInfo });
	};

	const handleSaveResult = async () => {
		if (analysisResults) {
			try {
				const saved = await PlantDiseaseService.saveAnalysisResult(
					analysisResults,
				);
				if (saved) {
					Alert.alert(
						'Saved',
						'Analysis result has been saved to your history.',
					);
				} else {
	...
	const handleViewTreatmentGuide = () => {
		Alert.alert(
			'Treatment Guide',
			'This would open a detailed treatment guide.',
		);
	};
		...
		return (
			<View style={styles.resultsContainer}>
				<View style={styles.healthStatusContainer}>
					...
					<View
						style={[
							...
							{
								backgroundColor:
									analysisResults.healthStatus === 'Healthy'
										? AppColors.primaryGreen
										: analysisResults.healthStatus === 'Critical'
										? AppColors.errorRed
										: AppColors.accentOrange,
							},
						]}
				...
				<Text style={styles.plantTypeText}>
					Plant Type:{' '}
					{analysisResults.plantType.charAt(0).toUpperCase() +
						analysisResults.plantType.slice(1)}
				</Text>
				...
				{analysisResults.diseases && analysisResults.diseases.length > 0 ? (
					analysisResults.diseases.map((disease, index) => (
						<ResultCard
							...
							databaseInfo={disease.databaseInfo}
